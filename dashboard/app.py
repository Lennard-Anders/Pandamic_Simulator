...
def serve_layout():
    ...
    return html.Div([
        ...
        html.Div([
            ...
            section_title("Detailed contacts"),
            html.P("Load the first 100 stored contacts from the selected local run. "
                   "Standard rows are episodes; FullRaw and legacy rows are contact steps. "
                   "SummaryOnly has no individual rows. This preview is a prefix, not a statistical sample."),
            dbc.Button("Preview contacts", id="contact-preview-btn", color="secondary", size="sm"),
            html.Div(id="contact-preview", style={"overflowX": "auto", "maxHeight": "450px"}),
...
@app.callback(
    Output("contact-preview", "children"),
    Input("contact-preview-btn", "n_clicks"),
    Input("file-dropdown", "value"),
    prevent_initial_call=True,
)
def preview_contacts(_clicks, filepath):
    # Changing runs clears stale output without opening any contact stream.
    if callback_context.triggered_id != "contact-preview-btn":
        return ""
    if not filepath or Path(filepath) not in [Path(path) for path in list_csvs()]:
        return "Select a discovered local run first; uploaded time-series files contain no contact dataset."
    try:
        rows = contact_preview(Path(filepath).parent)
    except (OSError, ValueError, EOFError, csv.Error) as error:
        return f"Contact preview unavailable: {error}"
    if not rows:
        return "No individual contacts stored for this run."
    columns = list(rows[0])
    return html.Div([
        html.P(f"Showing {len(rows)} rows at most; use aggregate files for whole-run statistics."),
        html.Table([
            html.Thead(html.Tr([html.Th(column) for column in columns])),
            html.Tbody([html.Tr([html.Td(row.get(column, "")) for column in columns]) for row in rows]),
