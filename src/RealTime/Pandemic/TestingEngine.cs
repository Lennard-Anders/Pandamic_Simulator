// <copyright file="TestingEngine.cs" company="dymanoid">
...
namespace RealTime.Pandemic
{
    ...
    internal enum PandemicTestState
    {
        Requested,
        Queued,
        Scheduled,
    ...
    internal enum PandemicTestResult
    {
        Unknown,
        Positive,
        Negative,
    ...
    internal enum PandemicTestPriority
    {
        Routine,
        Symptomatic,
    }

    internal enum PandemicTestReason
    {
        Screening,
        Symptoms,
        ContactTracing,
    ...
    /// <summary>The disease facts observable when a sample is actually taken.</summary>
    internal sealed class PandemicTestSampleContext
    {
    ...
    /// <summary>Complete lifecycle record for one test.</summary>
    internal sealed class PandemicTestRecord
    {
        ...
        public DateTime? ResultAvailableAt { get; set; }

        public PandemicTestResult Result { get; set; }

        public PandemicTestState State { get; set; }

        public PandemicTestPriority Priority { get; set; }

        public PandemicTestReason Reason { get; set; }

        public DiseaseState DiseaseStateAtSample { get; set; }

        public double? InfectionAgeDaysAtSample { get; set; }

        internal PandemicTestResult PendingResult { get; set; }
    }

    /// <summary>Explicit, unit-bearing policy for the testing state machine.</summary>
    internal sealed class PandemicTestingPolicy
    {
        public int Population { get; set; }

        public double RelativeCapacityPercentPerSevenDays { get; set; }

        public double ReservedForSymptomaticPercent { get; set; }

        public double MaximumRequestToSampleDays { get; set; }

        public double ResultDelayDays { get; set; }

        public double DetectionTimeDays { get; set; }

        public double SensitivityPercent { get; set; }

        public double SpecificityPercent { get; set; }

        public double RetestIntervalDays { get; set; }

        public double PositiveBlockingDays { get; set; }

        public bool QuarantineWhileAwaitingResult { get; set; }
        ...
        public void Validate()
        {
            ...
            ValidatePercent(RelativeCapacityPercentPerSevenDays, nameof(RelativeCapacityPercentPerSevenDays));
            ValidatePercent(ReservedForSymptomaticPercent, nameof(ReservedForSymptomaticPercent));
            ValidatePercent(SensitivityPercent, nameof(SensitivityPercent));
            ValidatePercent(SpecificityPercent, nameof(SpecificityPercent));
            ValidateFiniteNonnegative(MaximumRequestToSampleDays, nameof(MaximumRequestToSampleDays));
            ValidateFiniteNonnegative(ResultDelayDays, nameof(ResultDelayDays));
            ValidateFiniteNonnegative(DetectionTimeDays, nameof(DetectionTimeDays));
            ValidateFiniteNonnegative(RetestIntervalDays, nameof(RetestIntervalDays));
            ValidateFiniteNonnegative(PositiveBlockingDays, nameof(PositiveBlockingDays));
        }
    ...
    /// <summary>Unity-independent testing queue with no access to future results.</summary>
    internal sealed class TestingEngine
    {
        ...
        public TestingEngine(PandemicTestingPolicy policy, Random random, DateTime initialTime)
        {
